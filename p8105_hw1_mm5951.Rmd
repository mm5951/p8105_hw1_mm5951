...
Next, I will briefly describe the **key characteristics** of the **"penguins" dataset**. I'll first use the `skmr::skim`function to print out a summary of the dataset. The "penguins" dataset contains 344 rows and 8 columns. That corresponds to the data of n=8 different variables (columns) from n=344 penguins (rows).
...
Alternatively, as per homework instructions, I could have used the `nrow`and `ncol` functions to return the number of rows and columns of the specified matrix ("penguins"), respectively.
